...
Для дашбордов используется `Matrialized View`. 
Причины:
- Упрощение создания дашбордов - легче выразить SQL запросом в схеме
- Переносимость - можно быстро заменить провайдера BI инструмента
- Нет большой нагрузки на БД - значения пересчитываются интервалами

## Количество отмененных заказов за последний период

Показывает количество случаев отмены заказа за периоды: 1 день, 1 неделя и 1 месяц.
Фильтрация по причине отмены не производится.

Таблица: `orders_cancelled_last_day`, `orders_cancelled_last_week`, `orders_cancelled_last_month` - для дня, недели и месяца, соответственно

## Количество оформленных заказов за последний период

Показывает количество случаев оформления заказа за последний период: 1 день, 1 неделя и 1 месяц.
Дополнительная фильтрация (заказ в последствии был отменен и др.) не производится.

Таблицы: `orders_created_last_day`, `orders_created_last_week`, `orders_created_last_month`

## Самые популярные страницы

Показывает топ страниц по посещению.
Топ основывается на общем количестве посещений страниц, без учета конкретного пользователя.
Период - 1 месяц.

Таблица: `top_pages_month`

## Уникальные посетители (DAU, WAU, MAU)

Показывает какое количество уникальных посетителей зашло на сайт.
Учитывается сам факт захода на любую страницу, без учета времени провождения на сайте.
Периоды - 1 день, 1 неделя и 1 месяц.

> Это показатели Daily Active Users, Weekly Active Users и Monthly Active Users для дня, недели и месяца соответственно

Таблицы: `daily_active_users`, `weekly_active_users`, `monthly_active_users`

## Конверсионная воронка

**Статус: TODO**

Этот показатель состоит из нескольких метрик: просмотр товара, добавление в корзину, оформление заказа.
Показывает сколько покупателей в итоге купили товар.

Таблица: `conversion_funnel`
